...
const LeftMenu = () => {
  ...
  const [dataPrevious, setPreviousData] = useState(null);

  useEffect(() => {
    if (data && data?.user.email !== dataPrevious?.user.email) {
    ...
    <div className="w-25rem px-6 py-10 h-screen bg-primary-color text-white max-md:h-full max-md:py-8">
      ...
      <div
        className={`flex items-center cursor-pointer mt-10 ${
          pathname === "/" ? "div-menu-active-home" : "div-menu-home"
        }`}
        onClick={() => router.push("/")}
      >
        <i className="fa-solid fa-house-chimney"></i>
        <button className="font-semibold ml-3">Home</button>
      </div>
      <div
        className={`flex items-center cursor-pointer mt-10 ${
          pathname === "/request-off" ? "div-menu-active" : "div-menu"
        }`}
        onClick={() => router.push("/request-off")}
      >
        <img src="/calendar-menu.svg" alt="My Image" width="20" height="20" />
        <button className="font-semibold ml-2">Request time off</button>
      </div>
      {userInfo?.role === "admin" && (
        <div
          className={`flex items-center cursor-pointer mt-10 ${
            pathname === "/manage-request" ? "div-menu-active" : "div-menu"
          }`}
        ...
        >
          ...
          <button className="flex w-full justify-between font-semibold ml-2">
            ...
            {userInfo?.role === "admin" && (
              <p className="font-semibold text-cloud-color">
                {listRequestData?.length}
              </p>
            )}
          </button>
        </div>
      ...
      {userInfo?.role === "admin" ? (
        <div
          className={`flex items-center cursor-pointer mt-10 ${
            pathname === "/history-off" ? "div-menu-active" : "div-menu"
          }`}
          onClick={() => router.push("/history-off")}
        >
          <img src="/history-menu.svg" alt="My Image" width="20" height="20" />
          <button className="font-semibold ml-2">History day off</button>
        </div>
      ) : (
        <div
          className={`flex items-center cursor-pointer mt-10 ${
            pathname === "/history-off-member" ? "div-menu-active" : "div-menu"
          }`}
      ...
      {userInfo?.role === "admin" && (
        <div
          className={`flex items-center cursor-pointer mt-10 ${
            pathname === "/history-checkin" ? "div-menu-active" : "div-menu"
          }`}
          onClick={() => router.push("/history-checkin")}
        >
          <img src="/history-menu.svg" alt="My Image" width="20" height="20" />
          <button className="font-semibold ml-2">History Check-in</button>
        </div>
      )}
      {userInfo?.role === "admin" && (
        <div
          className={`flex items-center cursor-pointer mt-10 ${
            pathname === "/chart-dayoff" ? "div-menu-active" : "div-menu"
          }`}
          onClick={() => router.push("/chart-dayoff")}
        >
          <img src="/chart-menu.svg" alt="My Image" width="20" height="20" />
          <button className="font-semibold ml-2">Chart Dayoff Member</button>
        </div>
      )}
      <div
        className={`flex items-center cursor-pointer mt-10 ${
          pathname === "/logout" ? "div-menu-active" : "div-menu"
        }`}
        onClick={signOut}
      >
        <i className="fa-solid fa-arrow-right-from-bracket"></i>
        <button className="ml-4 font-semibold">Logout</button>
      </div>
    </div>
  );
};
